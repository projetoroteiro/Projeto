...
public class ProgramacaoDAO {
	...
	public static void editarProgramacao(Programacao programacao) {
		String sql="update programacao set dia=?, hora_ida=?, hora_volta=?, id_onibus=?, id_motorista=?, id_roteiro=? where id=?";
		try {
			PreparedStatement pstmt= con.prepareStatement(sql);
			pstmt.setDate(1, Date.valueOf(programacao.getDia()));;
			pstmt.setTime(2, Time.valueOf(programacao.getHoraIda()));
			...
			pstmt.setInt(5, programacao.getMotorista().getId());
			pstmt.setInt(6, programacao.getRoteiro().getId());
			pstmt.setInt(7, programacao.getId());
			pstmt.executeUpdate();
			pstmt.close(); 
		}catch (Exception e) {
			e.printStackTrace();
